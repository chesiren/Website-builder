...
(function()

  {
    ...
    //var e_fontsize = document.getElementById("e_fontsize");
    var e_imageurl = document.getElementById("e_imageurl");
    var e_alttext = document.getElementById("e_alttext");
    var e_bgcolor = document.getElementById("e_bgcolor");
    ...
    var editOptions = {
      "BODY": {
        ...
        "edit_text": true,
        "edit_font-size": true,
        "edit_text-color": true,
        "edit_text-align": true,
        "edit_size": true,
        "edit_background-color": true,
        "edit_src": true,
        "edit_padding": true,
        "edit_margin": true,
        "edit_border": true,
        "edit_border-radius": true,
        "edit_hover": true,
      },
      "P": [
        e_text
    ...
    // rgb2hex: https://stackoverflow.com/a/3627747
    const rgb2hex = (rgb) => `#${rgb.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/).slice(1).map(n => parseInt(n, 10).toString(16).padStart(2, '0')).join('')}`

    //doubleclick event to open edit modal
    document.addEventListener('dblclick', function (e) {
      ...
      if (e.target.getAttribute('draggable')) {
        ...
        var els = document.getElementsByClassName("edit_param");
        for(var el of els) {
          el.style.display = "none";
        }
        
        for(var f of editOptions[e.target.nodeName]) {
          f.style.display = "block";
        }

        switch (e.target.nodeName) {
          case "P":
            document.getElementById("p_area").value = curTarget.innerHTML;
          break;
          case "H1":
            document.getElementById("h1_area").value = curTarget.innerHTML;
          break;
          case "IMG":
            document.getElementById("img_url").value = curTarget.src;
            document.getElementById("p_alt").value = curTarget.alt;
          break;
          case "BUTTON":
            document.getElementById("p_area").value = curTarget.innerHTML;
            document.getElementById("bgcolorpicker").value = rgb2hex(window.getComputedStyle(curTarget, null).getPropertyValue("background-color"));
          break;
        }
      }
    });
    ...
    document.addEventListener('dragend', function(e) {
      ...
      if (selections.droptarget) {
        ...
        for (var len = selections.items.length, i = 0; i < len; i++) {
          // put a new object in the sidebar
          if (selections.items[i].parentNode.classList.contains("dragto")) {
            let item = selections.items[i].cloneNode(true);
            item.setAttribute("aria-grabbed", "false");
            selections.items[i].parentNode.appendChild(item);
          }
